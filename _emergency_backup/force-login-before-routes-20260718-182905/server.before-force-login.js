...
async function handsoffSupplierWriteGuard(req, res, next) {
  try {
    ...
    return res.status(403).json({
      message: "Bu tedarikÃ§i iÅŸlemi iÃ§in yetkiniz yok.",
    ...
    return res.status(500).json({
      message: "TedarikÃ§i yetki kontrolÃ¼ yapÄ±lamadÄ±.",
...
async function handsoffSettingsGuard(req, res, next) {
  try {
    ...
    return res.status(403).json({
      message: "Bu ayar iÅŸlemi iÃ§in yetkiniz yok.",
    ...
    return res.status(500).json({
      message: "Ayar yetki kontrolÃ¼ yapÄ±lamadÄ±.",
      detail: error.message,
    });
  }
}
...
app.get("/api/inventory-items", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok kartlarÄ± alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/inventory-items", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok kartÄ± kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/stock-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok hareketleri alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/purchase-orders/:id/create-stock-movement", authMiddleware, async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({
        message: "GeÃ§ersiz satÄ±n alma talebi ID.",
      });
    }
    ...
    if (!order) {
      return res.status(404).json({
        message: "SatÄ±n alma talebi bulunamadÄ±.",
      });
    }

    if (order.status === "CANCELLED") {
      return res.status(400).json({
        message: "Ä°ptal edilmiÅŸ talep stoÄŸa aktarÄ±lamaz.",
      });
    }

    if (order.stockMovementCreated) {
      return res.status(400).json({
        message: "Bu talep daha Ã¶nce stoÄŸa aktarÄ±lmÄ±ÅŸ.",
      });
    }
    ...
    const movement = await prisma.stockMovement.create({
      data: {
        ...
        note:
          order.supplierName +
          " satÄ±n alma talebinden stok giriÅŸi oluÅŸturuldu.",
      },
      include: {
        inventoryItem: true,
        purchaseOrder: true,
    ...
    return res.json({
      message: "SatÄ±n alma talebi stoÄŸa aktarÄ±ldÄ±.",
    ...
    return res.status(500).json({
      message: "SatÄ±n alma talebi stoÄŸa aktarÄ±lamadÄ±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/purchase-orders", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "SatÄ±n alma talepleri alÄ±namadÄ±.",
    });
  }
});
...
app.post("/api/purchase-orders", authMiddleware, async (req, res) => {
  try {
    ...
    if (!supplierName || !String(supplierName).trim()) {
      return res.status(400).json({
        message: "TedarikÃ§i adÄ± zorunludur.",
      });
    }

    if (!itemName || !String(itemName).trim()) {
      return res.status(400).json({
        message: "ÃœrÃ¼n / kalem adÄ± zorunludur.",
      });
    }
    ...
    const order = await prisma.purchaseOrder.create({
      data: {
        ...
        category: category || "Stok / SatÄ±n Alma",
    ...
    return res.json({
      message: "SatÄ±n alma talebi oluÅŸturuldu.",
    ...
    return res.status(500).json({
      message: "SatÄ±n alma talebi oluÅŸturulamadÄ±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/purchase-orders/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({
        message: "GeÃ§ersiz satÄ±n alma ID.",
      });
    }
    ...
    return res.json({
      message: "SatÄ±n alma talebi gÃ¼ncellendi.",
      purchaseOrder: formatPurchaseOrder(updatedOrder),
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      message: "SatÄ±n alma talebi gÃ¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.post("/api/purchase-orders/:id/create-expense", authMiddleware, async (req, res) => {
  try {
    ...
    if (order.expenseCreated) {
      return res.status(400).json({
        message: "Bu satÄ±n alma kaydÄ± daha Ã¶nce giderlere aktarÄ±lmÄ±ÅŸ.",
      });
    }

    if (order.status === "CANCELLED") {
      return res.status(400).json({
        message: "Ä°ptal edilmiÅŸ satÄ±n alma giderlere aktarÄ±lamaz.",
      });
    }
    ...
    const expense = await prisma.expense.create({
      data: {
        ...
        category: order.category || "Stok / SatÄ±n Alma",
        description:
          order.itemName +
          " - " +
          order.quantity +
        ...
        source: "PURCHASE_ORDER",
        note: order.note
          ? order.note + "\nSatÄ±n alma kaydÄ±ndan gider oluÅŸturuldu."
          : "SatÄ±n alma kaydÄ±ndan gider oluÅŸturuldu.",
      },
    });
    ...
    return res.json({
      message: "SatÄ±n alma kaydÄ± giderlere aktarÄ±ldÄ±.",
      purchaseOrder: formatPurchaseOrder(updatedOrder),
      expense,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "SatÄ±n alma giderlere aktarÄ±lamadÄ±.",
    });
  }
});
...
app.get("/api/stock-counts", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok sayÄ±mlarÄ± alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/stock-counts", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    return res.json({
      message: "Stok sayÄ±mÄ± oluÅŸturuldu.",
    ...
    return res.status(500).json({
      message: "Stok sayÄ±mÄ± oluÅŸturulamadÄ±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/stock-counts/:id/lines/:lineId", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!stockCount) {
      return res.status(404).json({
        message: "Stok sayÄ±mÄ± bulunamadÄ±.",
      });
    }

    if (stockCount.status === "COMPLETED") {
      return res.status(400).json({
        message: "TamamlanmÄ±ÅŸ stok sayÄ±mÄ± dÃ¼zenlenemez.",
      });
    }
    ...
    if (!line) {
      return res.status(404).json({
        message: "Stok sayÄ±m satÄ±rÄ± bulunamadÄ±.",
      });
    }
    ...
    return res.json({
      message: "Stok sayÄ±m satÄ±rÄ± gÃ¼ncellendi.",
    ...
    return res.status(500).json({
      message: "Stok sayÄ±m satÄ±rÄ± gÃ¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.post("/api/stock-counts/:id/complete", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (stockCount.status === "COMPLETED") {
      return res.status(400).json({
        message: "Bu stok sayÄ±mÄ± zaten tamamlanmÄ±ÅŸ.",
      });
    }
        ...
        if (difference !== 0) {
          await tx.stockMovement.create({
            data: {
              ...
              note:
                stockCount.countNo +
                " stok sayÄ±mÄ± fark dÃ¼zeltmesi. Sistem: " +
                line.systemStock +
                ", SayÄ±m: " +
                counted,
            },
    ...
    return res.json({
      message: "Stok sayÄ±mÄ± tamamlandÄ± ve stoklar gÃ¼ncellendi.",
    ...
    return res.status(500).json({
      message: "Stok sayÄ±mÄ± tamamlanamadÄ±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/waste-records", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Zayi / kÄ±rÄ±lma kayÄ±tlarÄ± alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/waste-records", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!itemName && !inventoryItemId) {
      return res.status(400).json({
        message: "ÃœrÃ¼n / stok adÄ± zorunludur.",
      });
    }

    if (numericQuantity <= 0) {
      return res.status(400).json({
        message: "Miktar 0'dan bÃ¼yÃ¼k olmalÄ±dÄ±r.",
      });
    }
    ...
    return res.json({
      message: inventoryItem && deductFromStock
        ? "Zayi / kÄ±rÄ±lma kaydÄ± oluÅŸturuldu ve stoktan dÃ¼ÅŸÃ¼ldÃ¼."
        : "Zayi / kÄ±rÄ±lma kaydÄ± oluÅŸturuldu.",
    ...
    return res.status(500).json({
      message: "Zayi / kÄ±rÄ±lma kaydÄ± oluÅŸturulamadÄ±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/waste-records/:id/cancel", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!record) {
      return res.status(404).json({
        message: "Zayi / kÄ±rÄ±lma kaydÄ± bulunamadÄ±.",
      });
    }

    if (record.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu kayÄ±t zaten iptal edilmiÅŸ.",
      });
    }
      ...
      if (record.inventoryItemId && record.stockDeducted) {
        ...
        await tx.stockMovement.create({
          data: {
            ...
            note: "Ä°ptal edilen zayi / kÄ±rÄ±lma kaydÄ± stoÄŸa geri alÄ±ndÄ±.",
          },
        });
      }
    ...
    return res.json({
      message: "Zayi / kÄ±rÄ±lma kaydÄ± iptal edildi.",
    ...
    return res.status(500).json({
      message: "Zayi / kÄ±rÄ±lma kaydÄ± iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
function getExpenseDebtAmountForStatement(expense) {
  ...
  if (paymentStatus === "PAID" || paymentStatus === "ODENDI" || paymentStatus === "Ã–DENDI" || paymentStatus === "Ã–DENDÄ°") {
    return 0;
  }
...
app.get("/api/supplier-payments", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "TedarikÃ§i Ã¶demeleri alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/supplier-payments", authMiddleware, async (req, res) => {
  try {
    ...
    if (numericAmount <= 0) {
      return res.status(400).json({
        message: "Ã–deme tutarÄ± 0'dan bÃ¼yÃ¼k olmalÄ±dÄ±r.",
      });
    }
    ...
    return res.json({
      message: "TedarikÃ§i Ã¶demesi kaydedildi.",
    ...
    return res.status(500).json({
      message: "TedarikÃ§i Ã¶demesi kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/supplier-payments/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!payment) {
      return res.status(404).json({
        message: "TedarikÃ§i Ã¶demesi bulunamadÄ±.",
      });
    }

    if (payment.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu Ã¶deme zaten iptal edilmiÅŸ.",
      });
    }
    ...
    return res.json({
      message: "TedarikÃ§i Ã¶demesi iptal edildi.",
    ...
    return res.status(500).json({
      message: "TedarikÃ§i Ã¶demesi iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/supplier-statements", authMiddleware, async (req, res) => {
  try {
    ...
    function ensureStatement({ supplierId, supplierName, supplier }) {
      ...
      if (!statementMap.has(key)) {
        statementMap.set(key, {
          ...
          supplierName: supplierName || "TedarikÃ§i belirtilmemiÅŸ",
    ...
    return res.status(500).json({
      message: "TedarikÃ§i cari kayÄ±tlarÄ± alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/cash-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Kasa hareketleri alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/cash-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message:
        finalDirection === "IN"
          ? "Kasa giriÅŸi kaydedildi."
          : "Kasa Ã§Ä±kÄ±ÅŸÄ± kaydedildi.",
...
app.put("/api/cash-movements/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!movement) {
      return res.status(404).json({
        message: "Kasa hareketi bulunamadÄ±.",
      });
    }

    if (movement.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu kasa hareketi zaten iptal edilmiÅŸ.",
      });
    }
...
app.get("/api/daily-sales", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃ¼nlÃ¼k ciro kayÄ±tlarÄ± alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/daily-sales", authMiddleware, async (req, res) => {
  try {
    ...
    if (totalAmount <= 0) {
      return res.status(400).json({
        message: "GÃ¼nlÃ¼k ciro toplamÄ± 0'dan bÃ¼yÃ¼k olmalÄ±dÄ±r.",
      });
    }
    ...
    return res.json({
      message: "GÃ¼nlÃ¼k ciro kaydÄ± oluÅŸturuldu.",
    ...
    return res.status(500).json({
      message: "GÃ¼nlÃ¼k ciro kaydÄ± oluÅŸturulamadÄ±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/daily-sales/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!sale) {
      return res.status(404).json({
        message: "GÃ¼nlÃ¼k ciro kaydÄ± bulunamadÄ±.",
      });
    }

    if (sale.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu gÃ¼nlÃ¼k ciro kaydÄ± zaten iptal edilmiÅŸ.",
      });
    }
    ...
    return res.json({
      message: "GÃ¼nlÃ¼k ciro kaydÄ± iptal edildi.",
    ...
    return res.status(500).json({
      message: "GÃ¼nlÃ¼k ciro kaydÄ± iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
/*
  Uyum endpointleri:
  Nakit AkÄ±ÅŸÄ± ve KÃ¢r Zarar ekranlarÄ± /api/sales veya /api/daily-reports okuyorsa
  aynÄ± gÃ¼nlÃ¼k ciro verisini bu endpointlerden de dÃ¶ndÃ¼rÃ¼yoruz.
*/
...
if (!global.__handsOffSalesCompatibilityRoutesAdded) {
  ...
  app.get("/api/sales", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: "SatÄ±ÅŸ kayÄ±tlarÄ± alÄ±namadÄ±.",
        detail: error.message,
      });
    }
  });
  ...
  app.get("/api/daily-reports", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: "GÃ¼nlÃ¼k rapor kayÄ±tlarÄ± alÄ±namadÄ±.",
        detail: error.message,
      });
    }
  });
...
async function readBackupModel(modelName, restaurantId, options = {}) {
  try {
    ...
    console.warn("Backup model okunamadÄ±:", modelName, error.message);
    return [];
  }
}
...
app.get("/api/restaurant-settings", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Restoran ayarlarÄ± alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/restaurant-settings", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "Restoran ayarlarÄ± kaydedildi.",
      settings,
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      message: "Restoran ayarlarÄ± kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/action-tasks", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Aksiyon gÃ¶revleri alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
  ...
  try {
    ...
    const task = await prisma.actionTask.create({
      data: {
        ...
        source: body.source ? String(body.source).trim() : "YÃ¶netim",
        priority: body.priority ? String(body.priority).trim() : "Orta",
        status: body.status ? String(body.status).trim() : "AÃ§Ä±k",
...
const HANDSOFF_DAILY_CHECKLIST_SECTIONS = [
  {
    title: "Ciro ve Kasa",
    items: [
      "GÃ¼nlÃ¼k ciro giriÅŸi yapÄ±ldÄ±",
      "Nakit / kart / online satÄ±ÅŸ ayrÄ±mÄ± kontrol edildi",
      "Kasa hareketleri kontrol edildi",
      "Eksik kasa Ã§Ä±kÄ±ÅŸÄ± varsa iÅŸlendi",
    ],
  },
  {
    title: "Gider ve TedarikÃ§i",
    items: [
      "GÃ¼nÃ¼n giderleri iÅŸlendi",
      "TedarikÃ§i Ã¶demeleri kontrol edildi",
      "AÃ§Ä±k cari borÃ§lar kontrol edildi",
      "SatÄ±n alma talepleri kontrol edildi",
    ],
  },
  {
    title: "Stok ve Operasyon",
    items: [
      "DÃ¼ÅŸÃ¼k stok uyarÄ±larÄ± kontrol edildi",
      "Zayi / kÄ±rÄ±lma kayÄ±tlarÄ± iÅŸlendi",
  ...
  },
  {
    title: "KapanÄ±ÅŸ",
    items: [
      "GÃ¼n sonu raporu kontrol edildi",
      "Veri yedeÄŸi alÄ±ndÄ±",
      "Ertesi gÃ¼n notlarÄ± yazÄ±ldÄ±",
    ],
  },
];
...
app.get("/api/daily-checklists/:date", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃ¼nlÃ¼k kontrol listesi alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/daily-checklists/:date", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "GÃ¼nlÃ¼k kontrol listesi kaydedildi.",
    ...
    return res.status(500).json({
      message: "GÃ¼nlÃ¼k kontrol listesi kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Rol listesi alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "KullanÄ±cÄ± rolÃ¼ eklendi.",
      user,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "KullanÄ±cÄ± rolÃ¼ eklenemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/user-roles/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!existing) {
      return res.status(404).json({
        message: "KullanÄ±cÄ± rolÃ¼ bulunamadÄ±.",
      });
    }
...
app.delete("/api/user-roles/:id", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "KullanÄ±cÄ± rolÃ¼ silindi.",
    });
  } catch (error) {
    console.error("user-roles rescue delete error:", error);

    return res.status(500).json({
      message: "KullanÄ±cÄ± rolÃ¼ silinemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "KullanÄ±cÄ± rolleri alÄ±namadÄ±.",
      detail: error.message,
    });
  }
});
