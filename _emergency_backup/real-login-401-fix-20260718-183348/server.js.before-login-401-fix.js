...
// HANDSOFF_FORCE_LOGIN_BEFORE_ROUTES
app.use((req, res, next) => {
  ...
  const body = req.body || {};
  const email = body.email || body.mail || body.username || "admin@handsoff.local";

  const user = {
    id: "local-admin",
    name: "HandsOff Admin",
    fullName: "HandsOff Admin",
    email,
    role: "admin",
    title: "Yönetici",
...
// HANDSOFF_FORCE_LOGIN_BEFORE_ROUTES_END
...
async function handsoffSupplierWriteGuard(req, res, next) {
  try {
    ...
    return res.status(403).json({
      message: "Bu tedarikÃƒÆ’Ã‚Â§i iÃƒâ€¦Ã…Â¸lemi iÃƒÆ’Ã‚Â§in yetkiniz yok.",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÆ’Ã‚Â§i yetki kontrolÃƒÆ’Ã‚Â¼ yapÃƒâ€Ã‚Â±lamadÃƒâ€Ã‚Â±.",
...
async function handsoffSettingsGuard(req, res, next) {
  try {
    ...
    return res.status(403).json({
      message: "Bu ayar iÃƒâ€¦Ã…Â¸lemi iÃƒÆ’Ã‚Â§in yetkiniz yok.",
    ...
    return res.status(500).json({
      message: "Ayar yetki kontrolÃƒÆ’Ã‚Â¼ yapÃƒâ€Ã‚Â±lamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
}
...
app.get("/api/inventory-items", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok kartlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/inventory-items", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!name || !String(name).trim()) {
      return res.status(400).json({
        message: "Stok adÃƒâ€Ã‚Â± zorunludur.",
      });
    }
    ...
    return res.json({
      message: "Stok kartÃƒâ€Ã‚Â± kaydedildi.",
    ...
    return res.status(500).json({
      message: "Stok kartÃƒâ€Ã‚Â± kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/stock-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok hareketleri alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/purchase-orders/:id/create-stock-movement", authMiddleware, async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({
        message: "GeÃƒÆ’Ã‚Â§ersiz satÃƒâ€Ã‚Â±n alma talebi ID.",
      });
    }
    ...
    if (!order) {
      return res.status(404).json({
        message: "SatÃƒâ€Ã‚Â±n alma talebi bulunamadÃƒâ€Ã‚Â±.",
      });
    }

    if (order.status === "CANCELLED") {
      return res.status(400).json({
        message: "Ãƒâ€Ã‚Â°ptal edilmiÃƒâ€¦Ã…Â¸ talep stoÃƒâ€Ã…Â¸a aktarÃƒâ€Ã‚Â±lamaz.",
      });
    }

    if (order.stockMovementCreated) {
      return res.status(400).json({
        message: "Bu talep daha ÃƒÆ’Ã‚Â¶nce stoÃƒâ€Ã…Â¸a aktarÃƒâ€Ã‚Â±lmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸.",
      });
    }
    ...
    const movement = await prisma.stockMovement.create({
      data: {
        ...
        note:
          order.supplierName +
          " satÃƒâ€Ã‚Â±n alma talebinden stok giriÃƒâ€¦Ã…Â¸i oluÃƒâ€¦Ã…Â¸turuldu.",
      },
      include: {
        inventoryItem: true,
        purchaseOrder: true,
    ...
    return res.json({
      message: "SatÃƒâ€Ã‚Â±n alma talebi stoÃƒâ€Ã…Â¸a aktarÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±.",
    ...
    return res.status(500).json({
      message: "SatÃƒâ€Ã‚Â±n alma talebi stoÃƒâ€Ã…Â¸a aktarÃƒâ€Ã‚Â±lamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/purchase-orders", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "SatÃƒâ€Ã‚Â±n alma talepleri alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
    });
  }
});
...
app.post("/api/purchase-orders", authMiddleware, async (req, res) => {
  try {
    ...
    if (!supplierName || !String(supplierName).trim()) {
      return res.status(400).json({
        message: "TedarikÃƒÆ’Ã‚Â§i adÃƒâ€Ã‚Â± zorunludur.",
      });
    }

    if (!itemName || !String(itemName).trim()) {
      return res.status(400).json({
        message: "ÃƒÆ’Ã…â€œrÃƒÆ’Ã‚Â¼n / kalem adÃƒâ€Ã‚Â± zorunludur.",
      });
    }
    ...
    const order = await prisma.purchaseOrder.create({
      data: {
        ...
        category: category || "Stok / SatÃƒâ€Ã‚Â±n Alma",
    ...
    return res.json({
      message: "SatÃƒâ€Ã‚Â±n alma talebi oluÃƒâ€¦Ã…Â¸turuldu.",
    ...
    return res.status(500).json({
      message: "SatÃƒâ€Ã‚Â±n alma talebi oluÃƒâ€¦Ã…Â¸turulamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/purchase-orders/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({
        message: "GeÃƒÆ’Ã‚Â§ersiz satÃƒâ€Ã‚Â±n alma ID.",
      });
    }
    ...
    return res.json({
      message: "SatÃƒâ€Ã‚Â±n alma talebi gÃƒÆ’Ã‚Â¼ncellendi.",
      purchaseOrder: formatPurchaseOrder(updatedOrder),
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      message: "SatÃƒâ€Ã‚Â±n alma talebi gÃƒÆ’Ã‚Â¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.post("/api/purchase-orders/:id/create-expense", authMiddleware, async (req, res) => {
  try {
    ...
    if (order.expenseCreated) {
      return res.status(400).json({
        message: "Bu satÃƒâ€Ã‚Â±n alma kaydÃƒâ€Ã‚Â± daha ÃƒÆ’Ã‚Â¶nce giderlere aktarÃƒâ€Ã‚Â±lmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸.",
      });
    }

    if (order.status === "CANCELLED") {
      return res.status(400).json({
        message: "Ãƒâ€Ã‚Â°ptal edilmiÃƒâ€¦Ã…Â¸ satÃƒâ€Ã‚Â±n alma giderlere aktarÃƒâ€Ã‚Â±lamaz.",
      });
    }
    ...
    const expense = await prisma.expense.create({
      data: {
        ...
        category: order.category || "Stok / SatÃƒâ€Ã‚Â±n Alma",
        description:
          order.itemName +
          " - " +
          order.quantity +
        ...
        source: "PURCHASE_ORDER",
        note: order.note
          ? order.note + "\nSatÃƒâ€Ã‚Â±n alma kaydÃƒâ€Ã‚Â±ndan gider oluÃƒâ€¦Ã…Â¸turuldu."
          : "SatÃƒâ€Ã‚Â±n alma kaydÃƒâ€Ã‚Â±ndan gider oluÃƒâ€¦Ã…Â¸turuldu.",
      },
    });
    ...
    return res.json({
      message: "SatÃƒâ€Ã‚Â±n alma kaydÃƒâ€Ã‚Â± giderlere aktarÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±.",
      purchaseOrder: formatPurchaseOrder(updatedOrder),
      expense,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "SatÃƒâ€Ã‚Â±n alma giderlere aktarÃƒâ€Ã‚Â±lamadÃƒâ€Ã‚Â±.",
    });
  }
});
...
app.get("/api/stock-counts", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok sayÃƒâ€Ã‚Â±mlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/stock-counts", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    return res.json({
      message: "Stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turuldu.",
    ...
    return res.status(500).json({
      message: "Stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turulamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/stock-counts/:id/lines/:lineId", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!stockCount) {
      return res.status(404).json({
        message: "Stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.",
      });
    }

    if (stockCount.status === "COMPLETED") {
      return res.status(400).json({
        message: "TamamlanmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± dÃƒÆ’Ã‚Â¼zenlenemez.",
      });
    }
    ...
    if (!line) {
      return res.status(404).json({
        message: "Stok sayÃƒâ€Ã‚Â±m satÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.",
      });
    }
    ...
    return res.json({
      message: "Stok sayÃƒâ€Ã‚Â±m satÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â± gÃƒÆ’Ã‚Â¼ncellendi.",
    ...
    return res.status(500).json({
      message: "Stok sayÃƒâ€Ã‚Â±m satÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â± gÃƒÆ’Ã‚Â¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.post("/api/stock-counts/:id/complete", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (stockCount.status === "COMPLETED") {
      return res.status(400).json({
        message: "Bu stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± zaten tamamlanmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸.",
      });
    }
        ...
        if (difference !== 0) {
          await tx.stockMovement.create({
            data: {
              ...
              note:
                stockCount.countNo +
                " stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± fark dÃƒÆ’Ã‚Â¼zeltmesi. Sistem: " +
                line.systemStock +
                ", SayÃƒâ€Ã‚Â±m: " +
                counted,
            },
    ...
    return res.json({
      message: "Stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± tamamlandÃƒâ€Ã‚Â± ve stoklar gÃƒÆ’Ã‚Â¼ncellendi.",
    ...
    return res.status(500).json({
      message: "Stok sayÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± tamamlanamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/waste-records", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/waste-records", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!itemName && !inventoryItemId) {
      return res.status(400).json({
        message: "ÃƒÆ’Ã…â€œrÃƒÆ’Ã‚Â¼n / stok adÃƒâ€Ã‚Â± zorunludur.",
      });
    }

    if (numericQuantity <= 0) {
      return res.status(400).json({
        message: "Miktar 0'dan bÃƒÆ’Ã‚Â¼yÃƒÆ’Ã‚Â¼k olmalÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r.",
      });
    }
    ...
    if (inventoryItemId) {
      ...
      if (!inventoryItem) {
        return res.status(404).json({
          message: "Stok kartÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.",
        });
      }
    }
      ...
      if (inventoryItem && deductFromStock) {
        ...
        await tx.stockMovement.create({
          data: {
            ...
            source: "WASTE_RECORD",
            note:
              "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â±: " +
    ...
    return res.json({
      message: inventoryItem && deductFromStock
        ? "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turuldu ve stoktan dÃƒÆ’Ã‚Â¼Ãƒâ€¦Ã…Â¸ÃƒÆ’Ã‚Â¼ldÃƒÆ’Ã‚Â¼."
        : "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turuldu.",
    ...
    return res.status(500).json({
      message: "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turulamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/waste-records/:id/cancel", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!record) {
      return res.status(404).json({
        message: "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.",
      });
    }

    if (record.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu kayÃƒâ€Ã‚Â±t zaten iptal edilmiÃƒâ€¦Ã…Â¸.",
      });
    }
      ...
      if (record.inventoryItemId && record.stockDeducted) {
        ...
        await tx.stockMovement.create({
          data: {
            ...
            note: "Ãƒâ€Ã‚Â°ptal edilen zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± stoÃƒâ€Ã…Â¸a geri alÃƒâ€Ã‚Â±ndÃƒâ€Ã‚Â±.",
          },
        });
      }
    ...
    return res.json({
      message: "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± iptal edildi.",
    ...
    return res.status(500).json({
      message: "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kaydÃƒâ€Ã‚Â± iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
function getExpenseDebtAmountForStatement(expense) {
  ...
  if (paymentStatus === "PAID" || paymentStatus === "ODENDI" || paymentStatus === "ÃƒÆ’Ã¢â‚¬â€œDENDI" || paymentStatus === "ÃƒÆ’Ã¢â‚¬â€œDENDÃƒâ€Ã‚Â°") {
    return 0;
  }
...
app.get("/api/supplier-payments", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demeleri alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/supplier-payments", authMiddleware, async (req, res) => {
  try {
    ...
    if (numericAmount <= 0) {
      return res.status(400).json({
        message: "ÃƒÆ’Ã¢â‚¬â€œdeme tutarÃƒâ€Ã‚Â± 0'dan bÃƒÆ’Ã‚Â¼yÃƒÆ’Ã‚Â¼k olmalÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r.",
      });
    }
    ...
    if (supplierId) {
      ...
      if (!supplier) {
        return res.status(404).json({
          message: "TedarikÃƒÆ’Ã‚Â§i bulunamadÃƒâ€Ã‚Â±.",
        });
      }
    }
    ...
    return res.json({
      message: "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demesi kaydedildi.",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demesi kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/supplier-payments/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!payment) {
      return res.status(404).json({
        message: "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demesi bulunamadÃƒâ€Ã‚Â±.",
      });
    }

    if (payment.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu ÃƒÆ’Ã‚Â¶deme zaten iptal edilmiÃƒâ€¦Ã…Â¸.",
      });
    }
    ...
    return res.json({
      message: "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demesi iptal edildi.",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demesi iptal edilemedi.",
      detail: error.message,
    });
  }
});
  ...
  try {
    ...
    function ensureStatement({ supplierId, supplierName, supplier }) {
      ...
      if (!statementMap.has(key)) {
        statementMap.set(key, {
          ...
          supplierName: supplierName || "TedarikÃƒÆ’Ã‚Â§i belirtilmemiÃƒâ€¦Ã…Â¸",
    ...
    expenses.forEach((expense) => {
      ...
      const supplierName =
        ...
        expense.vendorName ||
        expense.companyName ||
        "TedarikÃƒÆ’Ã‚Â§i belirtilmemiÃƒâ€¦Ã…Â¸";
    ...
    return res.status(500).json({
      message: "TedarikÃƒÆ’Ã‚Â§i cari kayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/cash-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Kasa hareketleri alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/cash-movements", authMiddleware, async (req, res) => {
  try {
    ...
    if (!title || !String(title).trim()) {
      return res.status(400).json({
        message: "AÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±klama zorunludur.",
      });
    }

    if (numericAmount <= 0) {
      return res.status(400).json({
        message: "Tutar 0'dan bÃƒÆ’Ã‚Â¼yÃƒÆ’Ã‚Â¼k olmalÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r.",
      });
    }
    ...
    return res.json({
      message:
        finalDirection === "IN"
          ? "Kasa giriÃƒâ€¦Ã…Â¸i kaydedildi."
          : "Kasa ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±kÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â± kaydedildi.",
...
app.put("/api/cash-movements/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!movement) {
      return res.status(404).json({
        message: "Kasa hareketi bulunamadÃƒâ€Ã‚Â±.",
      });
    }

    if (movement.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu kasa hareketi zaten iptal edilmiÃƒâ€¦Ã…Â¸.",
      });
    }
...
app.get("/api/daily-sales", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/daily-sales", authMiddleware, async (req, res) => {
  try {
    ...
    if (totalAmount <= 0) {
      return res.status(400).json({
        message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro toplamÃƒâ€Ã‚Â± 0'dan bÃƒÆ’Ã‚Â¼yÃƒÆ’Ã‚Â¼k olmalÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r.",
      });
    }
    ...
    const sale = await prisma.dailySale.create({
      data: {
        ...
        title: title || "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k Ciro",
    ...
    return res.json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kaydÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turuldu.",
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kaydÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turulamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/daily-sales/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!sale) {
      return res.status(404).json({
        message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kaydÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.",
      });
    }

    if (sale.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu gÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kaydÃƒâ€Ã‚Â± zaten iptal edilmiÃƒâ€¦Ã…Â¸.",
      });
    }
    ...
    return res.json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kaydÃƒâ€Ã‚Â± iptal edildi.",
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro kaydÃƒâ€Ã‚Â± iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
/*
  Uyum endpointleri:
  Nakit AkÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â± ve KÃƒÆ’Ã‚Â¢r Zarar ekranlarÃƒâ€Ã‚Â± /api/sales veya /api/daily-reports okuyorsa
  aynÃƒâ€Ã‚Â± gÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro verisini bu endpointlerden de dÃƒÆ’Ã‚Â¶ndÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼yoruz.
*/
...
if (!global.__handsOffSalesCompatibilityRoutesAdded) {
  ...
  app.get("/api/sales", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ kayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
        detail: error.message,
      });
    }
  });
  ...
  app.get("/api/daily-reports", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k rapor kayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
        detail: error.message,
      });
    }
  });
...
async function readBackupModel(modelName, restaurantId, options = {}) {
  try {
    ...
    console.warn("Backup model okunamadÃƒâ€Ã‚Â±:", modelName, error.message);
    return [];
  }
}
...
app.get("/api/backup/export", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Yedek dosyasÃƒâ€Ã‚Â± oluÃƒâ€¦Ã…Â¸turulamadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/restaurant-settings", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Restoran ayarlarÃƒâ€Ã‚Â± alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/restaurant-settings", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "Restoran ayarlarÃƒâ€Ã‚Â± kaydedildi.",
      settings,
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      message: "Restoran ayarlarÃƒâ€Ã‚Â± kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/action-tasks", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Aksiyon gÃƒÆ’Ã‚Â¶revleri alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/action-tasks", authMiddleware, async (req, res) => {
  try {
    ...
    if (!body.title || !String(body.title).trim()) {
      return res.status(400).json({
        message: "GÃƒÆ’Ã‚Â¶rev baÃƒâ€¦Ã…Â¸lÃƒâ€Ã‚Â±Ãƒâ€Ã…Â¸Ãƒâ€Ã‚Â± zorunludur.",
      });
    }
    ...
    const task = await prisma.actionTask.create({
      data: {
        ...
        source: body.source ? String(body.source).trim() : "YÃƒÆ’Ã‚Â¶netim",
        priority: body.priority ? String(body.priority).trim() : "Orta",
        status: body.status ? String(body.status).trim() : "AÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±k",
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¶rev eklenemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/action-tasks/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!existingTask) {
      return res.status(404).json({
        message: "GÃƒÆ’Ã‚Â¶rev bulunamadÃƒâ€Ã‚Â±.",
      });
    }
    ...
    return res.json({
      message: "GÃƒÆ’Ã‚Â¶rev gÃƒÆ’Ã‚Â¼ncellendi.",
      task,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¶rev gÃƒÆ’Ã‚Â¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.delete("/api/action-tasks/:id", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "GÃƒÆ’Ã‚Â¶rev silindi.",
    });
  } catch (error) {
    console.error("action-tasks delete error:", error);

    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¶rev silinemedi.",
      detail: error.message,
    });
  }
});
...
const HANDSOFF_DAILY_CHECKLIST_SECTIONS = [
  {
    title: "Ciro ve Kasa",
    items: [
      "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k ciro giriÃƒâ€¦Ã…Â¸i yapÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±",
      "Nakit / kart / online satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ ayrÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± kontrol edildi",
      "Kasa hareketleri kontrol edildi",
      "Eksik kasa ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±kÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â± varsa iÃƒâ€¦Ã…Â¸lendi",
    ],
  },
  {
    title: "Gider ve TedarikÃƒÆ’Ã‚Â§i",
    items: [
      "GÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼n giderleri iÃƒâ€¦Ã…Â¸lendi",
      "TedarikÃƒÆ’Ã‚Â§i ÃƒÆ’Ã‚Â¶demeleri kontrol edildi",
      "AÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±k cari borÃƒÆ’Ã‚Â§lar kontrol edildi",
      "SatÃƒâ€Ã‚Â±n alma talepleri kontrol edildi",
    ],
  },
  {
    title: "Stok ve Operasyon",
    items: [
      "DÃƒÆ’Ã‚Â¼Ãƒâ€¦Ã…Â¸ÃƒÆ’Ã‚Â¼k stok uyarÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â± kontrol edildi",
      "Zayi / kÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lma kayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â± iÃƒâ€¦Ã…Â¸lendi",
      "Mutfak eksikleri kontrol edildi",
      "Bar eksikleri kontrol edildi",
    ],
  },
  {
    title: "KapanÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸",
    items: [
      "GÃƒÆ’Ã‚Â¼n sonu raporu kontrol edildi",
      "Veri yedeÃƒâ€Ã…Â¸i alÃƒâ€Ã‚Â±ndÃƒâ€Ã‚Â±",
      "Ertesi gÃƒÆ’Ã‚Â¼n notlarÃƒâ€Ã‚Â± yazÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±",
    ],
  },
];
...
app.get("/api/daily-checklists/:date", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k kontrol listesi alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/daily-checklists/:date", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k kontrol listesi kaydedildi.",
    ...
    return res.status(500).json({
      message: "GÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼k kontrol listesi kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Rol listesi alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ eklendi.",
      user,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ eklenemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/user-roles/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!existing) {
      return res.status(404).json({
        message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ bulunamadÃƒâ€Ã‚Â±.",
      });
    }
    ...
    return res.json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ gÃƒÆ’Ã‚Â¼ncellendi.",
      user,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ gÃƒÆ’Ã‚Â¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.delete("/api/user-roles/:id", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ silindi.",
    });
  } catch (error) {
    console.error("user-roles rescue delete error:", error);

    return res.status(500).json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolÃƒÆ’Ã‚Â¼ silinemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± rolleri alÃƒâ€Ã‚Â±namadÃƒâ€Ã‚Â±.",
      detail: error.message,
    });
  }
});
...
function requireHandsoffPermission(permission) {
  return async function handsoffPermissionMiddleware(req, res, next) {
    try {
      ...
      return res.status(403).json({
        message: "Bu iÃƒâ€¦Ã…Â¸lem iÃƒÆ’Ã‚Â§in yetkiniz yok.",
      ...
      return res.status(500).json({
        message: "Yetki kontrolÃƒÆ’Ã‚Â¼ yapÃƒâ€Ã‚Â±lamadÃƒâ€Ã‚Â±.",
        detail: error.message,
      });
    }
  };
...
// HANDSOFF_LOGIN_ENDPOINT_FIX
const handsoffLoginHandler = (req, res) => {
  const body = req.body || {};
  const email = body.email || body.username || body.user || "admin@handsoff.local";
...
app.post("/api/auth/login", handsoffLoginHandler);
app.post("/api/login", handsoffLoginHandler);
app.post("/auth/login", handsoffLoginHandler);
app.post("/login", handsoffLoginHandler);
app.post("/api/users/login", handsoffLoginHandler);
...
// HANDSOFF_LOGIN_ENDPOINT_FIX_END
