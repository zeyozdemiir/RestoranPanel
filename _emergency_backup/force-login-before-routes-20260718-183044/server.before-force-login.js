...
async function handsoffSupplierWriteGuard(req, res, next) {
  try {
    ...
    return res.status(403).json({
      message: "Bu tedarikÃƒÂ§i iÃ…Å¸lemi iÃƒÂ§in yetkiniz yok.",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÂ§i yetki kontrolÃƒÂ¼ yapÃ„Â±lamadÃ„Â±.",
...
async function handsoffSettingsGuard(req, res, next) {
  try {
    ...
    return res.status(403).json({
      message: "Bu ayar iÃ…Å¸lemi iÃƒÂ§in yetkiniz yok.",
    ...
    return res.status(500).json({
      message: "Ayar yetki kontrolÃƒÂ¼ yapÃ„Â±lamadÃ„Â±.",
      detail: error.message,
    });
  }
}
...
app.get("/api/inventory-items", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok kartlarÃ„Â± alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/inventory-items", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    return res.json({
      message: "Stok kartÃ„Â± kaydedildi.",
    ...
    return res.status(500).json({
      message: "Stok kartÃ„Â± kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/stock-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok hareketleri alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/purchase-orders/:id/create-stock-movement", authMiddleware, async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({
        message: "GeÃƒÂ§ersiz satÃ„Â±n alma talebi ID.",
      });
    }
    ...
    if (!order) {
      return res.status(404).json({
        message: "SatÃ„Â±n alma talebi bulunamadÃ„Â±.",
      });
    }

    if (order.status === "CANCELLED") {
      return res.status(400).json({
        message: "Ã„Â°ptal edilmiÃ…Å¸ talep stoÃ„Å¸a aktarÃ„Â±lamaz.",
      });
    }

    if (order.stockMovementCreated) {
      return res.status(400).json({
        message: "Bu talep daha ÃƒÂ¶nce stoÃ„Å¸a aktarÃ„Â±lmÃ„Â±Ã…Å¸.",
      });
    }
    ...
    const movement = await prisma.stockMovement.create({
      data: {
        ...
        note:
          order.supplierName +
          " satÃ„Â±n alma talebinden stok giriÃ…Å¸i oluÃ…Å¸turuldu.",
      },
      include: {
        inventoryItem: true,
        purchaseOrder: true,
    ...
    return res.json({
      message: "SatÃ„Â±n alma talebi stoÃ„Å¸a aktarÃ„Â±ldÃ„Â±.",
    ...
    return res.status(500).json({
      message: "SatÃ„Â±n alma talebi stoÃ„Å¸a aktarÃ„Â±lamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/purchase-orders", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "SatÃ„Â±n alma talepleri alÃ„Â±namadÃ„Â±.",
    });
  }
});
...
app.post("/api/purchase-orders", authMiddleware, async (req, res) => {
  try {
    ...
    if (!supplierName || !String(supplierName).trim()) {
      return res.status(400).json({
        message: "TedarikÃƒÂ§i adÃ„Â± zorunludur.",
      });
    }

    if (!itemName || !String(itemName).trim()) {
      return res.status(400).json({
        message: "ÃƒÅ“rÃƒÂ¼n / kalem adÃ„Â± zorunludur.",
      });
    }
    ...
    const order = await prisma.purchaseOrder.create({
      data: {
        ...
        category: category || "Stok / SatÃ„Â±n Alma",
    ...
    return res.json({
      message: "SatÃ„Â±n alma talebi oluÃ…Å¸turuldu.",
    ...
    return res.status(500).json({
      message: "SatÃ„Â±n alma talebi oluÃ…Å¸turulamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/purchase-orders/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({
        message: "GeÃƒÂ§ersiz satÃ„Â±n alma ID.",
      });
    }
    ...
    return res.json({
      message: "SatÃ„Â±n alma talebi gÃƒÂ¼ncellendi.",
      purchaseOrder: formatPurchaseOrder(updatedOrder),
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      message: "SatÃ„Â±n alma talebi gÃƒÂ¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.post("/api/purchase-orders/:id/create-expense", authMiddleware, async (req, res) => {
  try {
    ...
    if (order.expenseCreated) {
      return res.status(400).json({
        message: "Bu satÃ„Â±n alma kaydÃ„Â± daha ÃƒÂ¶nce giderlere aktarÃ„Â±lmÃ„Â±Ã…Å¸.",
      });
    }

    if (order.status === "CANCELLED") {
      return res.status(400).json({
        message: "Ã„Â°ptal edilmiÃ…Å¸ satÃ„Â±n alma giderlere aktarÃ„Â±lamaz.",
      });
    }
    ...
    const expense = await prisma.expense.create({
      data: {
        ...
        category: order.category || "Stok / SatÃ„Â±n Alma",
        description:
          order.itemName +
          " - " +
          order.quantity +
        ...
        source: "PURCHASE_ORDER",
        note: order.note
          ? order.note + "\nSatÃ„Â±n alma kaydÃ„Â±ndan gider oluÃ…Å¸turuldu."
          : "SatÃ„Â±n alma kaydÃ„Â±ndan gider oluÃ…Å¸turuldu.",
      },
    });
    ...
    return res.json({
      message: "SatÃ„Â±n alma kaydÃ„Â± giderlere aktarÃ„Â±ldÃ„Â±.",
      purchaseOrder: formatPurchaseOrder(updatedOrder),
      expense,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "SatÃ„Â±n alma giderlere aktarÃ„Â±lamadÃ„Â±.",
    });
  }
});
...
app.get("/api/stock-counts", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Stok sayÃ„Â±mlarÃ„Â± alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/stock-counts", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    return res.json({
      message: "Stok sayÃ„Â±mÃ„Â± oluÃ…Å¸turuldu.",
    ...
    return res.status(500).json({
      message: "Stok sayÃ„Â±mÃ„Â± oluÃ…Å¸turulamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/stock-counts/:id/lines/:lineId", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!stockCount) {
      return res.status(404).json({
        message: "Stok sayÃ„Â±mÃ„Â± bulunamadÃ„Â±.",
      });
    }

    if (stockCount.status === "COMPLETED") {
      return res.status(400).json({
        message: "TamamlanmÃ„Â±Ã…Å¸ stok sayÃ„Â±mÃ„Â± dÃƒÂ¼zenlenemez.",
      });
    }
    ...
    if (!line) {
      return res.status(404).json({
        message: "Stok sayÃ„Â±m satÃ„Â±rÃ„Â± bulunamadÃ„Â±.",
      });
    }
    ...
    return res.json({
      message: "Stok sayÃ„Â±m satÃ„Â±rÃ„Â± gÃƒÂ¼ncellendi.",
    ...
    return res.status(500).json({
      message: "Stok sayÃ„Â±m satÃ„Â±rÃ„Â± gÃƒÂ¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.post("/api/stock-counts/:id/complete", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (stockCount.status === "COMPLETED") {
      return res.status(400).json({
        message: "Bu stok sayÃ„Â±mÃ„Â± zaten tamamlanmÃ„Â±Ã…Å¸.",
      });
    }
        ...
        if (difference !== 0) {
          await tx.stockMovement.create({
            data: {
              ...
              note:
                stockCount.countNo +
                " stok sayÃ„Â±mÃ„Â± fark dÃƒÂ¼zeltmesi. Sistem: " +
                line.systemStock +
                ", SayÃ„Â±m: " +
                counted,
            },
    ...
    return res.json({
      message: "Stok sayÃ„Â±mÃ„Â± tamamlandÃ„Â± ve stoklar gÃƒÂ¼ncellendi.",
    ...
    return res.status(500).json({
      message: "Stok sayÃ„Â±mÃ„Â± tamamlanamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/waste-records", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Zayi / kÃ„Â±rÃ„Â±lma kayÃ„Â±tlarÃ„Â± alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/waste-records", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!itemName && !inventoryItemId) {
      return res.status(400).json({
        message: "ÃƒÅ“rÃƒÂ¼n / stok adÃ„Â± zorunludur.",
      });
    }

    if (numericQuantity <= 0) {
      return res.status(400).json({
        message: "Miktar 0'dan bÃƒÂ¼yÃƒÂ¼k olmalÃ„Â±dÃ„Â±r.",
      });
    }
    ...
    if (inventoryItemId) {
      ...
      if (!inventoryItem) {
        return res.status(404).json({
          message: "Stok kartÃ„Â± bulunamadÃ„Â±.",
        });
      }
    }
    ...
    return res.json({
      message: inventoryItem && deductFromStock
        ? "Zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± oluÃ…Å¸turuldu ve stoktan dÃƒÂ¼Ã…Å¸ÃƒÂ¼ldÃƒÂ¼."
        : "Zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± oluÃ…Å¸turuldu.",
    ...
    return res.status(500).json({
      message: "Zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± oluÃ…Å¸turulamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/waste-records/:id/cancel", authMiddleware, requireHandsoffPermission("canManageStock"), async (req, res) => {
  try {
    ...
    if (!record) {
      return res.status(404).json({
        message: "Zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± bulunamadÃ„Â±.",
      });
    }

    if (record.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu kayÃ„Â±t zaten iptal edilmiÃ…Å¸.",
      });
    }
      ...
      if (record.inventoryItemId && record.stockDeducted) {
        ...
        await tx.stockMovement.create({
          data: {
            ...
            note: "Ã„Â°ptal edilen zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± stoÃ„Å¸a geri alÃ„Â±ndÃ„Â±.",
          },
        });
      }
    ...
    return res.json({
      message: "Zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± iptal edildi.",
    ...
    return res.status(500).json({
      message: "Zayi / kÃ„Â±rÃ„Â±lma kaydÃ„Â± iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
function getExpenseDebtAmountForStatement(expense) {
  ...
  if (paymentStatus === "PAID" || paymentStatus === "ODENDI" || paymentStatus === "Ãƒâ€“DENDI" || paymentStatus === "Ãƒâ€“DENDÃ„Â°") {
    return 0;
  }
...
app.get("/api/supplier-payments", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "TedarikÃƒÂ§i ÃƒÂ¶demeleri alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/supplier-payments", authMiddleware, async (req, res) => {
  try {
    ...
    if (numericAmount <= 0) {
      return res.status(400).json({
        message: "Ãƒâ€“deme tutarÃ„Â± 0'dan bÃƒÂ¼yÃƒÂ¼k olmalÃ„Â±dÃ„Â±r.",
      });
    }
    ...
    if (supplierId) {
      ...
      if (!supplier) {
        return res.status(404).json({
          message: "TedarikÃƒÂ§i bulunamadÃ„Â±.",
        });
      }
    }
    ...
    return res.json({
      message: "TedarikÃƒÂ§i ÃƒÂ¶demesi kaydedildi.",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÂ§i ÃƒÂ¶demesi kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/supplier-payments/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!payment) {
      return res.status(404).json({
        message: "TedarikÃƒÂ§i ÃƒÂ¶demesi bulunamadÃ„Â±.",
      });
    }

    if (payment.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu ÃƒÂ¶deme zaten iptal edilmiÃ…Å¸.",
      });
    }
    ...
    return res.json({
      message: "TedarikÃƒÂ§i ÃƒÂ¶demesi iptal edildi.",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÂ§i ÃƒÂ¶demesi iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/supplier-statements", authMiddleware, async (req, res) => {
  try {
    ...
    function ensureStatement({ supplierId, supplierName, supplier }) {
      ...
      if (!statementMap.has(key)) {
        statementMap.set(key, {
          ...
          supplierName: supplierName || "TedarikÃƒÂ§i belirtilmemiÃ…Å¸",
    ...
    return res.status(500).json({
      message: "TedarikÃƒÂ§i cari kayÃ„Â±tlarÃ„Â± alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/cash-movements", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Kasa hareketleri alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/cash-movements", authMiddleware, async (req, res) => {
  try {
    ...
    if (!title || !String(title).trim()) {
      return res.status(400).json({
        message: "AÃƒÂ§Ã„Â±klama zorunludur.",
      });
    }

    if (numericAmount <= 0) {
      return res.status(400).json({
        message: "Tutar 0'dan bÃƒÂ¼yÃƒÂ¼k olmalÃ„Â±dÃ„Â±r.",
      });
    }
    ...
    return res.json({
      message:
        finalDirection === "IN"
          ? "Kasa giriÃ…Å¸i kaydedildi."
          : "Kasa ÃƒÂ§Ã„Â±kÃ„Â±Ã…Å¸Ã„Â± kaydedildi.",
...
app.put("/api/cash-movements/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!movement) {
      return res.status(404).json({
        message: "Kasa hareketi bulunamadÃ„Â±.",
      });
    }

    if (movement.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu kasa hareketi zaten iptal edilmiÃ…Å¸.",
      });
    }
...
app.get("/api/daily-sales", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃƒÂ¼nlÃƒÂ¼k ciro kayÃ„Â±tlarÃ„Â± alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/daily-sales", authMiddleware, async (req, res) => {
  try {
    ...
    if (totalAmount <= 0) {
      return res.status(400).json({
        message: "GÃƒÂ¼nlÃƒÂ¼k ciro toplamÃ„Â± 0'dan bÃƒÂ¼yÃƒÂ¼k olmalÃ„Â±dÃ„Â±r.",
      });
    }
    ...
    return res.json({
      message: "GÃƒÂ¼nlÃƒÂ¼k ciro kaydÃ„Â± oluÃ…Å¸turuldu.",
    ...
    return res.status(500).json({
      message: "GÃƒÂ¼nlÃƒÂ¼k ciro kaydÃ„Â± oluÃ…Å¸turulamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/daily-sales/:id/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    if (!sale) {
      return res.status(404).json({
        message: "GÃƒÂ¼nlÃƒÂ¼k ciro kaydÃ„Â± bulunamadÃ„Â±.",
      });
    }

    if (sale.status === "CANCELLED") {
      return res.status(400).json({
        message: "Bu gÃƒÂ¼nlÃƒÂ¼k ciro kaydÃ„Â± zaten iptal edilmiÃ…Å¸.",
      });
    }
    ...
    return res.json({
      message: "GÃƒÂ¼nlÃƒÂ¼k ciro kaydÃ„Â± iptal edildi.",
    ...
    return res.status(500).json({
      message: "GÃƒÂ¼nlÃƒÂ¼k ciro kaydÃ„Â± iptal edilemedi.",
      detail: error.message,
    });
  }
});
...
/*
  Uyum endpointleri:
  Nakit AkÃ„Â±Ã…Å¸Ã„Â± ve KÃƒÂ¢r Zarar ekranlarÃ„Â± /api/sales veya /api/daily-reports okuyorsa
  aynÃ„Â± gÃƒÂ¼nlÃƒÂ¼k ciro verisini bu endpointlerden de dÃƒÂ¶ndÃƒÂ¼rÃƒÂ¼yoruz.
*/
...
if (!global.__handsOffSalesCompatibilityRoutesAdded) {
  ...
  app.get("/api/sales", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: "SatÃ„Â±Ã…Å¸ kayÃ„Â±tlarÃ„Â± alÃ„Â±namadÃ„Â±.",
        detail: error.message,
      });
    }
  });
  ...
  app.get("/api/daily-reports", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(500).json({
        message: "GÃƒÂ¼nlÃƒÂ¼k rapor kayÃ„Â±tlarÃ„Â± alÃ„Â±namadÃ„Â±.",
        detail: error.message,
      });
    }
  });
...
async function readBackupModel(modelName, restaurantId, options = {}) {
  try {
    ...
    console.warn("Backup model okunamadÃ„Â±:", modelName, error.message);
    return [];
  }
}
...
app.get("/api/backup/export", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Yedek dosyasÃ„Â± oluÃ…Å¸turulamadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.get("/api/restaurant-settings", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Restoran ayarlarÃ„Â± alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/restaurant-settings", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "Restoran ayarlarÃ„Â± kaydedildi.",
      settings,
    });
  } catch (error) {
    console.error(error);
    ...
    return res.status(500).json({
      message: "Restoran ayarlarÃ„Â± kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/action-tasks", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Aksiyon gÃƒÂ¶revleri alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/action-tasks", authMiddleware, async (req, res) => {
  try {
    ...
    if (!body.title || !String(body.title).trim()) {
      return res.status(400).json({
        message: "GÃƒÂ¶rev baÃ…Å¸lÃ„Â±Ã„Å¸Ã„Â± zorunludur.",
      });
    }
    ...
    const task = await prisma.actionTask.create({
      data: {
        ...
        source: body.source ? String(body.source).trim() : "YÃƒÂ¶netim",
        priority: body.priority ? String(body.priority).trim() : "Orta",
        status: body.status ? String(body.status).trim() : "AÃƒÂ§Ã„Â±k",
...
app.put("/api/action-tasks/:id", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃƒÂ¶rev gÃƒÂ¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
const HANDSOFF_DAILY_CHECKLIST_SECTIONS = [
  {
    title: "Ciro ve Kasa",
    items: [
      "GÃƒÂ¼nlÃƒÂ¼k ciro giriÃ…Å¸i yapÃ„Â±ldÃ„Â±",
      "Nakit / kart / online satÃ„Â±Ã…Å¸ ayrÃ„Â±mÃ„Â± kontrol edildi",
      "Kasa hareketleri kontrol edildi",
      "Eksik kasa ÃƒÂ§Ã„Â±kÃ„Â±Ã…Å¸Ã„Â± varsa iÃ…Å¸lendi",
    ],
  },
  {
    title: "Gider ve TedarikÃƒÂ§i",
    items: [
      "GÃƒÂ¼nÃƒÂ¼n giderleri iÃ…Å¸lendi",
      "TedarikÃƒÂ§i ÃƒÂ¶demeleri kontrol edildi",
      "AÃƒÂ§Ã„Â±k cari borÃƒÂ§lar kontrol edildi",
      "SatÃ„Â±n alma talepleri kontrol edildi",
    ],
  },
  {
    title: "Stok ve Operasyon",
    items: [
      "DÃƒÂ¼Ã…Å¸ÃƒÂ¼k stok uyarÃ„Â±larÃ„Â± kontrol edildi",
      "Zayi / kÃ„Â±rÃ„Â±lma kayÃ„Â±tlarÃ„Â± iÃ…Å¸lendi",
  ...
  },
  {
    title: "KapanÃ„Â±Ã…Å¸",
    items: [
      "GÃƒÂ¼n sonu raporu kontrol edildi",
      "Veri yedeÃ„Å¸i alÃ„Â±ndÃ„Â±",
      "Ertesi gÃƒÂ¼n notlarÃ„Â± yazÃ„Â±ldÃ„Â±",
    ],
  },
];
...
app.get("/api/daily-checklists/:date", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "GÃƒÂ¼nlÃƒÂ¼k kontrol listesi alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.put("/api/daily-checklists/:date", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "GÃƒÂ¼nlÃƒÂ¼k kontrol listesi kaydedildi.",
    ...
    return res.status(500).json({
      message: "GÃƒÂ¼nlÃƒÂ¼k kontrol listesi kaydedilemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Rol listesi alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
app.post("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ eklendi.",
      user,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ eklenemedi.",
      detail: error.message,
    });
  }
});
...
app.put("/api/user-roles/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!existing) {
      return res.status(404).json({
        message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ bulunamadÃ„Â±.",
      });
    }
    ...
    return res.json({
      message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ gÃƒÂ¼ncellendi.",
      user,
    });
  } catch (error) {
    ...
    return res.status(500).json({
      message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ gÃƒÂ¼ncellenemedi.",
      detail: error.message,
    });
  }
});
...
app.delete("/api/user-roles/:id", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({
      message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ silindi.",
    });
  } catch (error) {
    console.error("user-roles rescue delete error:", error);

    return res.status(500).json({
      message: "KullanÃ„Â±cÃ„Â± rolÃƒÂ¼ silinemedi.",
      detail: error.message,
    });
  }
});
...
app.get("/api/user-roles", authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "KullanÃ„Â±cÃ„Â± rolleri alÃ„Â±namadÃ„Â±.",
      detail: error.message,
    });
  }
});
...
function requireHandsoffPermission(permission) {
  return async function handsoffPermissionMiddleware(req, res, next) {
    try {
      ...
      return res.status(403).json({
        message: "Bu iÃ…Å¸lem iÃƒÂ§in yetkiniz yok.",
      ...
      return res.status(500).json({
        message: "Yetki kontrolÃƒÂ¼ yapÃ„Â±lamadÃ„Â±.",
        detail: error.message,
      });
    }
  };
