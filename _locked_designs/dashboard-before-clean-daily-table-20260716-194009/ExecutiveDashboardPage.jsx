  ...
  return (
    <div style={styles.dailyBreakdown}>
      <div style={styles.dailyTable}>
        ...
        {rows.map((row) => (
          <div key={row.date} style={styles.dailyRow}>
            <div style={styles.dayCell}>
              <strong>{row.day}</strong>
              <small>{row.date}</small>
            </div>

            <span style={styles.amountCell}>{formatCurrency(row.cash)}</span>
            <span style={styles.amountCell}>{formatCurrency(row.card)}</span>
            <span style={styles.amountCell}>{formatCurrency(row.online)}</span>
            ...
            <div style={styles.totalCell}>
              ...
              <div style={styles.miniBarTrack}>
                <div
                  style={{
                    ...styles.miniBarFill,
                    width: `${Math.max(8, (row.total / maxTotal) * 100)}%`,
                  }}
                />
              </div>
            </div>
      ...
      <aside style={styles.sideSummary}>
        <span style={styles.totalLabel}>Haftalık Toplam Gelir</span>
        <strong style={styles.totalValue}>{formatCurrency(grandTotal)}</strong>

        <div style={styles.paymentList}>
          {paymentTypes.map((item) => (
            <div key={item.label} style={styles.paymentItem}>
              <div>
                <span style={styles.paymentText}>{item.label}</span>
                <small style={styles.paymentPercent}>
                  %{Math.round((item.value / grandTotal) * 100)}
                </small>
              </div>
              <strong style={styles.paymentAmount}>{formatCurrency(item.value)}</strong>
            </div>
          ))}
        </div>

        <div style={styles.sideNote}>
          Günlük gelirler ödeme tiplerine göre ayrılmıştır.
        </div>
      </aside>
    </div>
  );
...
const styles = {
  ...
  dailyHeader: {
    display: "grid",
    gridTemplateColumns: "1.1fr 1fr 1fr 1fr 1.25fr",
    gap: "10px",
    padding: "0 12px 6px",
    color: "#94a3b8",
    fontSize: "12px",
